    ...
    $.fn.mauGallery.methods = {
        ...
        prevImage () {
            ...
            $(".item-column img").each(function() {
                let img = $(this);
                if (activeTag === "all" || img.data("gallery-tag") === activeTag) imagesCollection.push(img.attr("src")); // Stocker seulement les attributs src
            });
            // Trouver l'index de l'image active
            let activeIndex = imagesCollection.findIndex((src)=>src === activeImageSrc);
            // Calculer l'index de l'image précédente, boucler si nécessaire
        ...
        nextImage () {
            ...
            // Construction de la collection d'images
            $(".item-column img").each(function() {
                if (activeTag === "all" || $(this).data("gallery-tag") === activeTag) imagesCollection.push($(this).attr("src")); // Stocker seulement les attributs src
            });
            // Trouver l'index de l'image active
            let activeIndex = imagesCollection.findIndex((src)=>src === activeImageSrc);
            // Déterminer l'image suivante
            let nextIndex = (activeIndex + 1) % imagesCollection.length; // S'assure de boucler à la première image
            let nextImageSrc = imagesCollection[nextIndex];
            // Mettre à jour l'attribut src de la lightbox avec l'image suivante
        ...
        filterByTag () {
            if ($(this).hasClass("active active-tag")) return;
...
//# sourceMappingURL=index.566e85bb.js.map
